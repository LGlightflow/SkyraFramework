...
#include "SkyraRangedWeaponInstance.h"
...
void USkyraRangedWeaponInstance::PostLoad()
{
	Super::PostLoad();
...
#if WITH_EDITOR
void USkyraRangedWeaponInstance::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
	UpdateDebugVisualization();
}

void USkyraRangedWeaponInstance::UpdateDebugVisualization()
{
...
void USkyraRangedWeaponInstance::OnEquipped()
{
	Super::OnEquipped();
...
void USkyraRangedWeaponInstance::OnUnequipped()
{
	Super::OnUnequipped();
}

void USkyraRangedWeaponInstance::Tick(float DeltaSeconds)
{
	APawn* Pawn = GetPawn();
	check(Pawn != nullptr);
...
void USkyraRangedWeaponInstance::ComputeHeatRange(float& MinHeat, float& MaxHeat)
{
	float Min1;
	float Max1;
...
void USkyraRangedWeaponInstance::ComputeSpreadRange(float& MinSpread, float& MaxSpread)
{
	HeatToSpreadCurve.GetRichCurveConst()->GetValueRange(/*out*/ MinSpread, /*out*/ MaxSpread);
}

void USkyraRangedWeaponInstance::AddSpread()
{
...
float USkyraRangedWeaponInstance::GetDistanceAttenuation(float Distance, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
	const FRichCurve* Curve = DistanceDamageFalloff.GetRichCurveConst();
	return Curve->HasAnyData() ? Curve->Eval(Distance) : 1.0f;
}

float USkyraRangedWeaponInstance::GetPhysicalMaterialAttenuation(const UPhysicalMaterial* PhysicalMaterial, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
...
bool USkyraRangedWeaponInstance::UpdateSpread(float DeltaSeconds)
{
...
bool USkyraRangedWeaponInstance::UpdateMultipliers(float DeltaSeconds)
{
	...
	if (const USkyraCameraComponent* CameraComponent = USkyraCameraComponent::FindCameraComponent(Pawn))
	{
		...
		AimingAlpha = (TopCameraTag == TAG_Skyra_Weapon_SteadyAimingCamera) ? TopCameraWeight : 0.0f;
	}

...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SkyraWeaponInstance)

class UAnimInstance;
struct FGameplayTagContainer;

USkyraWeaponInstance::USkyraWeaponInstance(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	...
	if (APawn* Pawn = GetPawn())
	{
		...
		{
			if (USkyraHealthComponent* HealthComponent = USkyraHealthComponent::FindHealthComponent(GetPawn()))
			{
...
void USkyraWeaponInstance::OnEquipped()
{
	Super::OnEquipped();
...
void USkyraWeaponInstance::OnUnequipped()
{
	Super::OnUnequipped();
...
void USkyraWeaponInstance::UpdateFiringTime()
{
	UWorld* World = GetWorld();
	check(World);
...
float USkyraWeaponInstance::GetTimeSinceLastInteractedWith() const
{
	UWorld* World = GetWorld();
	check(World);
...
TSubclassOf<UAnimInstance> USkyraWeaponInstance::PickBestAnimLayer(bool bEquipped, const FGameplayTagContainer& CosmeticTags) const
{
	const FSkyraAnimLayerSelectionSet& SetToQuery = (bEquipped ? EquippedAnimSet : UneuippedAnimSet);
	return SetToQuery.SelectBestLayer(CosmeticTags);
}

const FPlatformUserId USkyraWeaponInstance::GetOwningUserId() const
{
...
void USkyraWeaponInstance::ApplyDeviceProperties()
{
...
void USkyraWeaponInstance::RemoveDeviceProperties()
{
	const FPlatformUserId UserId = GetOwningUserId();
...
void USkyraWeaponInstance::OnDeathStarted(AActor* OwningActor)
{

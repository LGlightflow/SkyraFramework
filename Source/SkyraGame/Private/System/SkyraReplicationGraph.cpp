...
/**
*	
*	===================== SkyraReplicationGraph Replication =====================
*
*	Overview
*	
*		This changes the way actor relevancy works. AActor::IsNetRelevantFor is NOT used in this system!
*		
*		Instead, The USkyraReplicationGraph contains UReplicationGraphNodes. These nodes are responsible for generating lists of actors to replicate for each connection.
...
*		
*		USkyraReplicationGraph is intended to not be directly used by the game code. That is, you should not have to include SkyraReplicationGraph.h anywhere else.
*		Rather, USkyraReplicationGraph depends on the game code and registers for events that the game code broadcasts (e.g., events for players joining/leaving teams).
*		This choice was made because it gives USkyraReplicationGraph a complete holistic view of actor replication. Rather than exposing generic public functions that any
*		place in game code can invoke, all notifications are explicitly registered in USkyraReplicationGraph::InitGlobalActorClassSettings.
*		
*	Skyra Nodes
*	
...
*		
*		USkyraReplicationGraphNode_AlwaysRelevant_ForConnection
...
*		
*		USkyraReplicationGraphNode_PlayerStateFrequencyLimiter
...
*		owning connection only) via USkyraReplicationGraphNode_AlwaysRelevant_ForConnection.
*		
...
*		
*		Making something always relevant to connection: You will need to modify USkyraReplicationGraphNode_AlwaysRelevant_ForConnection::GatherActorListsForConnection. You will also want 
*		to make sure the actor does not get put in one of the other nodes. The safest way to do this is by setting its EClassRepNodeMapping to NotRouted in USkyraReplicationGraph::InitGlobalActorClassSettings.
*
*	How To Debug
*	
...
*		
*		Skyra.RepGraph.PrintRouting - will print the EClassRepNodeMapping for each class. That is, how a given actor class is routed (or not) in the Replication Graph.
*	
*/

#include "SkyraReplicationGraph.h"
...
DEFINE_LOG_CATEGORY( LogSkyraRepGraph );

namespace Skyra::RepGraph
{
	float DestructionInfoMaxDist = 30000.f;
	static FAutoConsoleVariableRef CVarSkyraRepGraphDestructMaxDist(TEXT("Skyra.RepGraph.DestructInfo.MaxDist"), DestructionInfoMaxDist, TEXT("Max distance (not squared) to rep destruct infos at"), ECVF_Default);

	int32 DisplayClientLevelStreaming = 0;
	static FAutoConsoleVariableRef CVarSkyraRepGraphDisplayClientLevelStreaming(TEXT("Skyra.RepGraph.DisplayClientLevelStreaming"), DisplayClientLevelStreaming, TEXT(""), ECVF_Default);

	float CellSize = 10000.f;
	static FAutoConsoleVariableRef CVarSkyraRepGraphCellSize(TEXT("Skyra.RepGraph.CellSize"), CellSize, TEXT(""), ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarSkyraRepGraphSpatialBiasX(TEXT("Skyra.RepGraph.SpatialBiasX"), SpatialBiasX, TEXT(""), ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarSkyraRepSpatialBiasY(TEXT("Skyra.RepGraph.SpatialBiasY"), SpatialBiasY, TEXT(""), ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarSkyraRepDynamicActorFrequencyBuckets(TEXT("Skyra.RepGraph.DynamicActorFrequencyBuckets"), DynamicActorFrequencyBuckets, TEXT(""), ECVF_Default);

	int32 DisableSpatialRebuilds = 1;
	static FAutoConsoleVariableRef CVarSkyraRepDisableSpatialRebuilds(TEXT("Skyra.RepGraph.DisableSpatialRebuilds"), DisableSpatialRebuilds, TEXT(""), ECVF_Default);

	int32 LogLazyInitClasses = 0;
	static FAutoConsoleVariableRef CVarSkyraRepLogLazyInitClasses(TEXT("Skyra.RepGraph.LogLazyInitClasses"), LogLazyInitClasses, TEXT(""), ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarSkyraRepTargetKBytesSecFastSharedPath(TEXT("Skyra.RepGraph.TargetKBytesSecFastSharedPath"), TargetKBytesSecFastSharedPath, TEXT(""), ECVF_Default);

	float FastSharedPathCullDistPct = 0.80f;
	static FAutoConsoleVariableRef CVarSkyraRepFastSharedPathCullDistPct(TEXT("Skyra.RepGraph.FastSharedPathCullDistPct"), FastSharedPathCullDistPct, TEXT(""), ECVF_Default);

	int32 EnableFastSharedPath = 1;
	static FAutoConsoleVariableRef CVarSkyraRepEnableFastSharedPath(TEXT("Skyra.RepGraph.EnableFastSharedPath"), EnableFastSharedPath, TEXT(""), ECVF_Default);
	...
	UReplicationDriver* ConditionalCreateReplicationDriver(UNetDriver* ForNetDriver, UWorld* World)
	{
		...
		{
			const USkyraReplicationGraphSettings* SkyraRepGraphSettings = GetDefault<USkyraReplicationGraphSettings>();

			// Enable/Disable via developer settings
			if (SkyraRepGraphSettings && SkyraRepGraphSettings->bDisableReplicationGraph)
			{
				UE_LOG(LogSkyraRepGraph, Display, TEXT("Replication graph is disabled via SkyraReplicationGraphSettings."));
				return nullptr;
			}

			UE_LOG(LogSkyraRepGraph, Display, TEXT("Replication graph is enabled for %s in world %s."), *GetNameSafe(ForNetDriver), *GetPathNameSafe(World));

			TSubclassOf<USkyraReplicationGraph> GraphClass = SkyraRepGraphSettings->DefaultReplicationGraphClass.TryLoadClass<USkyraReplicationGraph>();
			if (GraphClass.Get() == nullptr)
			{
				GraphClass = USkyraReplicationGraph::StaticClass();
			}

			USkyraReplicationGraph* SkyraReplicationGraph = NewObject<USkyraReplicationGraph>(GetTransientPackage(), GraphClass.Get());
			return SkyraReplicationGraph;
		}
...
USkyraReplicationGraph::USkyraReplicationGraph()
{
	if (!UReplicationDriver::CreateReplicationDriverDelegate().IsBound())
	{
		UReplicationDriver::CreateReplicationDriverDelegate().BindLambda(
			...
			{
				return Skyra::RepGraph::ConditionalCreateReplicationDriver(ForNetDriver, World);
			});
	}
}

void USkyraReplicationGraph::ResetGameWorldState()
{
	...
	for (UNetReplicationGraphConnection* ConnManager : Connections)
	{
		...
		{
			if (USkyraReplicationGraphNode_AlwaysRelevant_ForConnection* AlwaysRelevantConnectionNode = Cast<USkyraReplicationGraphNode_AlwaysRelevant_ForConnection>(ConnectionNode))
			{
...
EClassRepNodeMapping USkyraReplicationGraph::GetClassNodeMapping(UClass* Class) const
{
	if (!Class)
	{
...
void USkyraReplicationGraph::RegisterClassRepNodeMapping(UClass* Class)
{
	EClassRepNodeMapping Mapping = GetClassNodeMapping(Class);
	ClassRepNodePolicies.Set(Class, Mapping);
}

void USkyraReplicationGraph::InitClassReplicationInfo(FClassReplicationInfo& Info, UClass* Class, bool Spatialize) const
{
	AActor* CDO = Class->GetDefaultObject<AActor>();
	if (Spatialize)
	{
		Info.SetCullDistanceSquared(CDO->NetCullDistanceSquared);
		UE_LOG(LogSkyraRepGraph, Log, TEXT("Setting cull distance for %s to %f (%f)"), *Class->GetName(), Info.GetCullDistanceSquared(), Info.GetCullDistance());
	}
	...
	UE_LOG(LogSkyraRepGraph, Log, TEXT("Setting replication period for %s (%s) to %d frames (%.2f)"), *Class->GetName(), *NativeClass->GetName(), Info.ReplicationPeriodFrame, CDO->NetUpdateFrequency);
}

bool USkyraReplicationGraph::ConditionalInitClassReplicationInfo(UClass* ReplicatedClass, FClassReplicationInfo& ClassInfo)
{
...
void USkyraReplicationGraph::AddClassRepInfo(UClass* Class, EClassRepNodeMapping Mapping)
{
	if (IsSpatialized(Mapping))
	{
		if (Class->GetDefaultObject<AActor>()->bAlwaysRelevant)
		{
			UE_LOG(LogSkyraRepGraph, Warning, TEXT("Replicated Class %s is AlwaysRelevant but is initialized into a spatialized node (%s)"), *Class->GetName(), *StaticEnum<EClassRepNodeMapping>()->GetNameStringByValue((int64)Mapping));
		}
	}
...
void USkyraReplicationGraph::RegisterClassReplicationInfo(UClass* ReplicatedClass)
{
	...
	if (ConditionalInitClassReplicationInfo(ReplicatedClass, ClassInfo))
	{
		...
		UE_LOG(LogSkyraRepGraph, Log, TEXT("Setting %s - %.2f"), *GetNameSafe(ReplicatedClass), ClassInfo.GetCullDistance());
	}
}

void USkyraReplicationGraph::InitGlobalActorClassSettings()
{
	...
	GlobalActorReplicationInfoMap.SetInitClassInfoFunc(
		[this](UClass* Class, FClassReplicationInfo& ClassInfo)
		{
			...
			if (Skyra::RepGraph::LogLazyInitClasses != 0)
			{
				if (bHandled)
				{
					EClassRepNodeMapping Mapping = ClassRepNodePolicies.GetChecked(Class);
					UE_LOG(LogSkyraRepGraph, Warning, TEXT("%s was Lazy Initialized. (Parent: %s) %d."), *GetNameSafe(Class), *GetNameSafe(Class->GetSuperClass()), (int32)Mapping);
					...
					{
						UE_LOG(LogSkyraRepGraph, Warning, TEXT("Differences Found!"));
						FString DebugStr = ParentRepInfo.BuildDebugStringDelta();
						UE_LOG(LogSkyraRepGraph, Warning, TEXT("  Parent: %s"), *DebugStr);

						DebugStr = ClassInfo.BuildDebugStringDelta();
						UE_LOG(LogSkyraRepGraph, Warning, TEXT("  Class : %s"), *DebugStr);
					}
				}
				else
				{
					UE_LOG(LogSkyraRepGraph, Warning, TEXT("%s skipped Lazy Initialization because it does not differ from its parent. (Parent: %s)"), *GetNameSafe(Class), *GetNameSafe(Class->GetSuperClass()));
	...
	const USkyraReplicationGraphSettings* SkyraRepGraphSettings = GetDefault<USkyraReplicationGraphSettings>();
	check(SkyraRepGraphSettings);

	// Set Classes Node Mappings
	for (const FRepGraphActorClassSettings& ActorClassSettings : SkyraRepGraphSettings->ClassSettings)
	{
		if (ActorClassSettings.bAddClassRepInfoToMap)
		{
			if (UClass* StaticActorClass = ActorClassSettings.GetStaticActorClass())
			{
				UE_LOG(LogSkyraRepGraph, Log, TEXT("ActorClassSettings -- AddClassRepInfo - %s :: %i"), *StaticActorClass->GetName(), int(ActorClassSettings.ClassNodeMapping));
...
#if WITH_GAMEPLAY_DEBUGGER
	AddClassRepInfo(AGameplayDebuggerCategoryReplicator::StaticClass(), EClassRepNodeMapping::NotRouted);				// Replicated via USkyraReplicationGraphNode_AlwaysRelevant_ForConnection
#endif
	...
	CharacterClassRepInfo.SetCullDistanceSquared(ASkyraCharacter::StaticClass()->GetDefaultObject<ASkyraCharacter>()->NetCullDistanceSquared);
...

	{
		...
		FRepMovement DefaultRepMovement = ASkyraCharacter::StaticClass()->GetDefaultObject<ASkyraCharacter>()->GetReplicatedMovement(); // Use the same quantization settings as our default replicatedmovement
	...
	{
		bool bSuccess = false;
		if (ASkyraCharacter* Character = Cast<ASkyraCharacter>(Actor))
		{
	...
	FastSharedPathConstants.MaxBitsPerFrame = (int32)((float)(Skyra::RepGraph::TargetKBytesSecFastSharedPath * 1024 * 8) / NetDriver->GetNetServerMaxTickRate());
	FastSharedPathConstants.DistanceRequirementPct = Skyra::RepGraph::FastSharedPathCullDistPct;

	SetClassInfo(ASkyraCharacter::StaticClass(), CharacterClassRepInfo);

	// ---------------------------------------------------------------------
	UReplicationGraphNode_ActorListFrequencyBuckets::DefaultSettings.ListSize = 12;
	UReplicationGraphNode_ActorListFrequencyBuckets::DefaultSettings.NumBuckets = Skyra::RepGraph::DynamicActorFrequencyBuckets;
	UReplicationGraphNode_ActorListFrequencyBuckets::DefaultSettings.BucketThresholds.Reset();
	UReplicationGraphNode_ActorListFrequencyBuckets::DefaultSettings.EnableFastPath = (Skyra::RepGraph::EnableFastSharedPath > 0);
	...
	UE_LOG(LogSkyraRepGraph, Log, TEXT(""));
	UE_LOG(LogSkyraRepGraph, Log, TEXT("Class Routing Map: "));
	for (auto ClassMapIt = ClassRepNodePolicies.CreateIterator(); ClassMapIt; ++ClassMapIt)
	{
		...
		UE_LOG(LogSkyraRepGraph, Log, TEXT("  %s (%s) -> %s"), *Class->GetName(), *GetNameSafe(ParentNativeClass), *StaticEnum<EClassRepNodeMapping>()->GetNameStringByValue((int64)Mapping));
	}

	UE_LOG(LogSkyraRepGraph, Log, TEXT(""));
	UE_LOG(LogSkyraRepGraph, Log, TEXT("Class Settings Map: "));
	...
	for (auto ClassRepInfoIt = GlobalActorReplicationInfoMap.CreateClassMapIterator(); ClassRepInfoIt; ++ClassRepInfoIt)
	{
		...
		UE_LOG(LogSkyraRepGraph, Log, TEXT("  %s (%s) -> %s"), *Class->GetName(), *GetNameSafe(GetParentNativeClass(Class)), *ClassInfo.BuildDebugStringDelta());
	}


	// Rep destruct infos based on CVar value
	DestructInfoMaxDistanceSquared = Skyra::RepGraph::DestructionInfoMaxDist * Skyra::RepGraph::DestructionInfoMaxDist;
	...
	for (const FRepGraphActorClassSettings& ActorClassSettings : SkyraRepGraphSettings->ClassSettings)
	{
		if (ActorClassSettings.bAddToRPC_Multicast_OpenChannelForClassMap)
		{
			if (UClass* StaticActorClass = ActorClassSettings.GetStaticActorClass())
			{
				UE_LOG(LogSkyraRepGraph, Log, TEXT("ActorClassSettings -- RPC_Multicast_OpenChannelForClass - %s"), *StaticActorClass->GetName());
...
void USkyraReplicationGraph::InitGlobalGraphNodes()
{
	// -----------------------------------------------
	//	Spatial Actors
	// -----------------------------------------------
	...
	GridNode->CellSize = Skyra::RepGraph::CellSize;
	GridNode->SpatialBias = FVector2D(Skyra::RepGraph::SpatialBiasX, Skyra::RepGraph::SpatialBiasY);

	if (Skyra::RepGraph::DisableSpatialRebuilds)
	{
	...
	// -----------------------------------------------
	USkyraReplicationGraphNode_PlayerStateFrequencyLimiter* PlayerStateNode = CreateNewNode<USkyraReplicationGraphNode_PlayerStateFrequencyLimiter>();
	AddGlobalGraphNode(PlayerStateNode);
}

void USkyraReplicationGraph::InitConnectionGraphNodes(UNetReplicationGraphConnection* RepGraphConnection)
{
	Super::InitConnectionGraphNodes(RepGraphConnection);

	USkyraReplicationGraphNode_AlwaysRelevant_ForConnection* AlwaysRelevantConnectionNode = CreateNewNode<USkyraReplicationGraphNode_AlwaysRelevant_ForConnection>();

	// This node needs to know when client levels go in and out of visibility
	RepGraphConnection->OnClientVisibleLevelNameAdd.AddUObject(AlwaysRelevantConnectionNode, &USkyraReplicationGraphNode_AlwaysRelevant_ForConnection::OnClientLevelVisibilityAdd);
	RepGraphConnection->OnClientVisibleLevelNameRemove.AddUObject(AlwaysRelevantConnectionNode, &USkyraReplicationGraphNode_AlwaysRelevant_ForConnection::OnClientLevelVisibilityRemove);

	AddConnectionGraphNode(AlwaysRelevantConnectionNode, RepGraphConnection);
}

EClassRepNodeMapping USkyraReplicationGraph::GetMappingPolicy(UClass* Class)
{
...
void USkyraReplicationGraph::RouteAddNetworkActorToNodes(const FNewReplicatedActorInfo& ActorInfo, FGlobalActorReplicationInfo& GlobalInfo)
{
...
void USkyraReplicationGraph::RouteRemoveNetworkActorToNodes(const FNewReplicatedActorInfo& ActorInfo)
{
	EClassRepNodeMapping Policy = GetMappingPolicy(ActorInfo.Class);
	switch(Policy)
	{
		...
		case EClassRepNodeMapping::RelevantAllConnections:
		{
			...
			else
			{
				...
				{
					UE_LOG(LogSkyraRepGraph, Warning, TEXT("Actor %s was not found in AlwaysRelevantStreamingLevelActors list. LevelName: %s"), *GetActorRepListTypeDebugString(ActorInfo.Actor), *ActorInfo.StreamingLevelName.ToString());
				}				
			}
...
#if WITH_GAMEPLAY_DEBUGGER
void USkyraReplicationGraph::OnGameplayDebuggerOwnerChange(AGameplayDebuggerCategoryReplicator* Debugger, APlayerController* OldOwner)
{
	CHECK_WORLDS(Debugger);

	auto GetAlwaysRelevantForConnectionNode = [this](APlayerController* Controller) -> USkyraReplicationGraphNode_AlwaysRelevant_ForConnection*
	{
		if (Controller)
		{
	...
	if (USkyraReplicationGraphNode_AlwaysRelevant_ForConnection* AlwaysRelevantConnectionNode = GetAlwaysRelevantForConnectionNode(OldOwner))
	{
		AlwaysRelevantConnectionNode->GameplayDebugger = nullptr;
	}

	if (USkyraReplicationGraphNode_AlwaysRelevant_ForConnection* AlwaysRelevantConnectionNode = GetAlwaysRelevantForConnectionNode(Debugger->GetReplicationOwner()))
	{
...
void USkyraReplicationGraphNode_AlwaysRelevant_ForConnection::ResetGameWorldState()
{
	ReplicationActorList.Reset();
	AlwaysRelevantStreamingLevelsNeedingReplication.Empty();
}

void USkyraReplicationGraphNode_AlwaysRelevant_ForConnection::GatherActorListsForConnection(const FConnectionGatherActorListParameters& Params)
{
	USkyraReplicationGraph* SkyraGraph = CastChecked<USkyraReplicationGraph>(GetOuter());
	...
	for (const FNetViewer& CurViewer : Params.Viewers)
	{
		...
		if (ASkyraPlayerController* PC = Cast<ASkyraPlayerController>(CurViewer.InViewer))
		{
			...
			if (bReplicatePS)
			{
				// Always return the player state to the owning player. Simulated proxy player states are handled by USkyraReplicationGraphNode_PlayerStateFrequencyLimiter
			...
			if (ASkyraCharacter* Pawn = Cast<ASkyraCharacter>(PC->GetPawn()))
			{
			...
			if (ASkyraCharacter* ViewTargetPawn = Cast<ASkyraCharacter>(CurViewer.ViewTarget))
			{
	...
	TMap<FName, FActorRepListRefView>& AlwaysRelevantStreamingLevelActors = SkyraGraph->AlwaysRelevantStreamingLevelActors;
	...
	for (int32 Idx=AlwaysRelevantStreamingLevelsNeedingReplication.Num()-1; Idx >= 0; --Idx)
	{
		...
		if (Ptr == nullptr)
		{
			...
			UE_CLOG(Skyra::RepGraph::DisplayClientLevelStreaming > 0, LogSkyraRepGraph, Display, TEXT("CLIENTSTREAMING Removing %s from AlwaysRelevantStreamingLevelActors because FActorRepListRefView is null. %s "), *StreamingLevel.ToString(),  *Params.ConnectionManager.GetName());
		...
		if (RepList.Num() > 0)
		{
			...
			if (bAllDormant)
			{
				UE_CLOG(Skyra::RepGraph::DisplayClientLevelStreaming > 0, LogSkyraRepGraph, Display, TEXT("CLIENTSTREAMING All AlwaysRelevant Actors Dormant on StreamingLevel %s for %s. Removing list."), *StreamingLevel.ToString(), *Params.ConnectionManager.GetName());
				AlwaysRelevantStreamingLevelsNeedingReplication.RemoveAtSwap(Idx, 1, EAllowShrinking::No);
			}
			else
			{
				UE_CLOG(Skyra::RepGraph::DisplayClientLevelStreaming > 0, LogSkyraRepGraph, Display, TEXT("CLIENTSTREAMING Adding always Actors on StreamingLevel %s for %s because it has at least one non dormant actor"), *StreamingLevel.ToString(), *Params.ConnectionManager.GetName());
				Params.OutGatheredReplicationLists.AddReplicationActorList(RepList);
			}
		}
		else
		{
			UE_LOG(LogSkyraRepGraph, Warning, TEXT("USkyraReplicationGraphNode_AlwaysRelevant_ForConnection::GatherActorListsForConnection - empty RepList %s"), *Params.ConnectionManager.GetName());
		}
...
void USkyraReplicationGraphNode_AlwaysRelevant_ForConnection::OnClientLevelVisibilityAdd(FName LevelName, UWorld* StreamingWorld)
{
	UE_CLOG(Skyra::RepGraph::DisplayClientLevelStreaming > 0, LogSkyraRepGraph, Display, TEXT("CLIENTSTREAMING ::OnClientLevelVisibilityAdd - %s"), *LevelName.ToString());
	AlwaysRelevantStreamingLevelsNeedingReplication.Add(LevelName);
}

void USkyraReplicationGraphNode_AlwaysRelevant_ForConnection::OnClientLevelVisibilityRemove(FName LevelName)
{
	UE_CLOG(Skyra::RepGraph::DisplayClientLevelStreaming > 0, LogSkyraRepGraph, Display, TEXT("CLIENTSTREAMING ::OnClientLevelVisibilityRemove - %s"), *LevelName.ToString());
	AlwaysRelevantStreamingLevelsNeedingReplication.Remove(LevelName);
}

void USkyraReplicationGraphNode_AlwaysRelevant_ForConnection::LogNode(FReplicationGraphDebugInfo& DebugInfo, const FString& NodeName) const
{
	...
	{
		USkyraReplicationGraph* SkyraGraph = CastChecked<USkyraReplicationGraph>(GetOuter());
		if (FActorRepListRefView* RepList = SkyraGraph->AlwaysRelevantStreamingLevelActors.Find(LevelName))
		{
...
USkyraReplicationGraphNode_PlayerStateFrequencyLimiter::USkyraReplicationGraphNode_PlayerStateFrequencyLimiter()
{
	bRequiresPrepareForReplicationCall = true;
}

void USkyraReplicationGraphNode_PlayerStateFrequencyLimiter::PrepareForReplication()
{
...
void USkyraReplicationGraphNode_PlayerStateFrequencyLimiter::GatherActorListsForConnection(const FConnectionGatherActorListParameters& Params)
{
...
void USkyraReplicationGraphNode_PlayerStateFrequencyLimiter::LogNode(FReplicationGraphDebugInfo& DebugInfo, const FString& NodeName) const
{
	DebugInfo.Log(NodeName);
	DebugInfo.PushIndent();	
...
void USkyraReplicationGraph::PrintRepNodePolicies()
{
	...
	GLog->Logf(TEXT("===================================="));
	GLog->Logf(TEXT("Skyra Replication Routing Policies"));
	GLog->Logf(TEXT("===================================="));
...
FAutoConsoleCommandWithWorldAndArgs SkyraPrintRepNodePoliciesCmd(TEXT("Skyra.RepGraph.PrintRouting"),TEXT("Prints how actor classes are routed to RepGraph nodes"),
	FConsoleCommandWithWorldAndArgsDelegate::CreateLambda([](const TArray<FString>& Args, UWorld* World)
	{
		for (TObjectIterator<USkyraReplicationGraph> It; It; ++It)
		{
...
FAutoConsoleCommandWithWorldAndArgs ChangeFrequencyBucketsCmd(TEXT("Skyra.RepGraph.FrequencyBuckets"), TEXT("Resets frequency bucket count."), FConsoleCommandWithWorldAndArgsDelegate::CreateLambda([](const TArray< FString >& Args, UWorld* World) 
{
	int32 Buckets = 1;
	if (Args.Num() > 0)
	{
	...
	UE_LOG(LogSkyraRepGraph, Display, TEXT("Setting Frequency Buckets to %d"), Buckets);

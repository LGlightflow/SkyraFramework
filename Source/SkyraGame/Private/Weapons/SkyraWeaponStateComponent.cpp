...
#include "SkyraWeaponStateComponent.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SkyraWeaponStateComponent)

UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_Gameplay_Zone, "Gameplay.Zone");

USkyraWeaponStateComponent::USkyraWeaponStateComponent(const FObjectInitializer& ObjectInitializer)
...
void USkyraWeaponStateComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (APawn* Pawn = GetPawn<APawn>())
	{
		...
		{
			if (USkyraRangedWeaponInstance* CurrentWeapon = Cast<USkyraRangedWeaponInstance>(EquipmentManager->GetFirstInstanceOfType(USkyraRangedWeaponInstance::StaticClass())))
			{
...
bool USkyraWeaponStateComponent::ShouldShowHitAsSuccess(const FHitResult& Hit) const
{
...
bool USkyraWeaponStateComponent::ShouldUpdateDamageInstigatedTime(const FGameplayEffectContextHandle& EffectContext) const
{
...
void USkyraWeaponStateComponent::ClientConfirmTargetData_Implementation(uint16 UniqueId, bool bSuccess, const TArray<uint8>& HitReplaces)
{
	for (int i = 0; i < UnconfirmedServerSideHitMarkers.Num(); i++)
	{
		FSkyraServerSideHitMarkerBatch& Batch = UnconfirmedServerSideHitMarkers[i];
		if (Batch.UniqueId == UniqueId)
		{
			if (bSuccess && (HitReplaces.Num() != Batch.Markers.Num()))
			{
				...
				for (const FSkyraScreenSpaceHitLocation& Entry : Batch.Markers)
				{
...
void USkyraWeaponStateComponent::AddUnconfirmedServerSideHitMarkers(const FGameplayAbilityTargetDataHandle& InTargetData, const TArray<FHitResult>& FoundHits)
{
	FSkyraServerSideHitMarkerBatch& NewUnconfirmedHitMarker = UnconfirmedServerSideHitMarkers.Emplace_GetRef(InTargetData.UniqueId);
	...
	if (APlayerController* OwnerPC = GetController<APlayerController>())
	{
		for (const FHitResult& Hit : FoundHits)
		{
			...
			{
				FSkyraScreenSpaceHitLocation& Entry = NewUnconfirmedHitMarker.Markers.AddDefaulted_GetRef();
...
void USkyraWeaponStateComponent::UpdateDamageInstigatedTime(const FGameplayEffectContextHandle& EffectContext)
{
...
void USkyraWeaponStateComponent::ActuallyUpdateDamageInstigatedTime()
{
...
double USkyraWeaponStateComponent::GetTimeSinceLastHitNotification() const
{
	UWorld* World = GetWorld();

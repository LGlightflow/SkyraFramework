...
#include "SkyraGameplayAbility_RangedWeapon.h"
#include "Weapons/SkyraRangedWeaponInstance.h"
#include "Physics/SkyraCollisionChannels.h"
#include "SkyraLogChannels.h"
#include "AIController.h"
#include "NativeGameplayTags.h"
#include "Weapons/SkyraWeaponStateComponent.h"
#include "AbilitySystemComponent.h"
#include "AbilitySystem/SkyraGameplayAbilityTargetData_SingleTargetHit.h"
#include "DrawDebugHelpers.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(SkyraGameplayAbility_RangedWeapon)
...
namespace SkyraConsoleVariables
{
	...
	static FAutoConsoleVariableRef CVarDrawBulletTraceDuraton(
		TEXT("skyra.Weapon.DrawBulletTraceDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHits(
		TEXT("skyra.Weapon.DrawBulletHitDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHitRadius(
		TEXT("skyra.Weapon.DrawBulletHitRadius"),
		DrawBulletHitRadius,
...
USkyraGameplayAbility_RangedWeapon::USkyraGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SourceBlockedTags.AddTag(TAG_WeaponFireBlocked);
}

USkyraRangedWeaponInstance* USkyraGameplayAbility_RangedWeapon::GetWeaponInstance() const
{
	return Cast<USkyraRangedWeaponInstance>(GetAssociatedEquipment());
}

bool USkyraGameplayAbility_RangedWeapon::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	if (bResult)
	{
		...
		{
			UE_LOG(LogSkyraAbilitySystem, Error, TEXT("Weapon ability %s cannot be activated because there is no associated ranged weapon (equipment instance=%s but needs to be derived from %s)"),
				*GetPathName(),
				*GetPathNameSafe(GetAssociatedEquipment()),
				*USkyraRangedWeaponInstance::StaticClass()->GetName());
			bResult = false;
		}
	}
...
int32 USkyraGameplayAbility_RangedWeapon::FindFirstPawnHitResult(const TArray<FHitResult>& HitResults)
{
...
void USkyraGameplayAbility_RangedWeapon::AddAdditionalTraceIgnoreActors(FCollisionQueryParams& TraceParams) const
{
...
ECollisionChannel USkyraGameplayAbility_RangedWeapon::DetermineTraceChannel(FCollisionQueryParams& TraceParams, bool bIsSimulated) const
{
	return Skyra_TraceChannel_Weapon;
}

FHitResult USkyraGameplayAbility_RangedWeapon::WeaponTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHitResults) const
{
...
FVector USkyraGameplayAbility_RangedWeapon::GetWeaponTargetingSourceLocation() const
{
...
FTransform USkyraGameplayAbility_RangedWeapon::GetTargetingTransform(APawn* SourcePawn, ESkyraAbilityTargetingSource Source) const
{
	check(SourcePawn);
	AController* SourcePawnController = SourcePawn->GetController(); 
	USkyraWeaponStateComponent* WeaponStateComponent = (SourcePawnController != nullptr) ? SourcePawnController->FindComponentByClass<USkyraWeaponStateComponent>() : nullptr;

	// The caller should determine the transform without calling this if the mode is custom!
	check(Source != ESkyraAbilityTargetingSource::Custom);
	...
	if ((Controller != nullptr) && ((Source == ESkyraAbilityTargetingSource::CameraTowardsFocus) || (Source == ESkyraAbilityTargetingSource::PawnTowardsFocus) || (Source == ESkyraAbilityTargetingSource::WeaponTowardsFocus)))
	{
		...
		if (Source == ESkyraAbilityTargetingSource::CameraTowardsFocus)
		{
	...
	if ((Source == ESkyraAbilityTargetingSource::WeaponForward) || (Source == ESkyraAbilityTargetingSource::WeaponTowardsFocus))
	{
		SourceLoc = GetWeaponTargetingSourceLocation();
	}
	else
	...
	if (bFoundFocus && ((Source == ESkyraAbilityTargetingSource::PawnTowardsFocus) || (Source == ESkyraAbilityTargetingSource::WeaponTowardsFocus)))
	{
...
FHitResult USkyraGameplayAbility_RangedWeapon::DoSingleBulletTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHits) const
{
#if ENABLE_DRAW_DEBUG
	if (SkyraConsoleVariables::DrawBulletTracesDuration > 0.0f)
	{
		static float DebugThickness = 1.0f;
		DrawDebugLine(GetWorld(), StartTrace, EndTrace, FColor::Red, false, SkyraConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
	}
...
void USkyraGameplayAbility_RangedWeapon::PerformLocalTargeting(OUT TArray<FHitResult>& OutHits)
{
	...
	USkyraRangedWeaponInstance* WeaponData = GetWeaponInstance();
	if (AvatarPawn && AvatarPawn->IsLocallyControlled() && WeaponData)
	{
		...
		if (SkyraConsoleVariables::DrawBulletTracesDuration > 0.0f)
		{
			...
			DrawDebugLine(GetWorld(), InputData.StartTrace, InputData.StartTrace + (InputData.AimDir * 100.0f), FColor::Yellow, false, SkyraConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
		}
#endif
...
void USkyraGameplayAbility_RangedWeapon::TraceBulletsInCartridge(const FRangedWeaponFiringInput& InputData, OUT TArray<FHitResult>& OutHits)
{
	USkyraRangedWeaponInstance* WeaponData = InputData.WeaponData;
	check(WeaponData);
	...
	for (int32 BulletIndex = 0; BulletIndex < BulletsPerCartridge; ++BulletIndex)
	{
		...
		{
#if ENABLE_DRAW_DEBUG
			if (SkyraConsoleVariables::DrawBulletHitDuration > 0.0f)
			{
				DrawDebugPoint(GetWorld(), Impact.ImpactPoint, SkyraConsoleVariables::DrawBulletHitRadius, FColor::Red, false, SkyraConsoleVariables::DrawBulletHitRadius);
			}
#endif
...
void USkyraGameplayAbility_RangedWeapon::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
	...
	if (const FGameplayAbilitySpec* AbilitySpec = MyAbilityComponent->FindAbilitySpecFromHandle(CurrentSpecHandle))
	{
		...
		if (!bProjectileWeapon)
		{
			if (AController* Controller = GetControllerFromActorInfo())
			{
				...
				{
					// Confirm hit markers
					if (USkyraWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<USkyraWeaponStateComponent>())
					{
		...
		{
			UE_LOG(LogSkyraAbilitySystem, Warning, TEXT("Weapon ability %s failed to commit (bIsTargetDataValid=%d)"), *GetPathName(), bIsTargetDataValid ? 1 : 0);
			K2_EndAbility();
		}
	}
...
void USkyraGameplayAbility_RangedWeapon::StartRangedWeaponTargeting()
{
	...
	check(Controller);
	USkyraWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<USkyraWeaponStateComponent>();
	...
	if (FoundHits.Num() > 0)
	{
		...
		{
			FSkyraGameplayAbilityTargetData_SingleTargetHit* NewTargetData = new FSkyraGameplayAbilityTargetData_SingleTargetHit();

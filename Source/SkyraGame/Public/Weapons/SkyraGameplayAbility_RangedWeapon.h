...
#include "SkyraGameplayAbility_RangedWeapon.generated.h"

enum ECollisionChannel : int;

class APawn;
class USkyraRangedWeaponInstance;
class UObject;
...
UENUM(BlueprintType)
enum class ESkyraAbilityTargetingSource : uint8
{
 ...
 */
UCLASS()
class USkyraGameplayAbility_RangedWeapon : public USkyraGameplayAbility_FromEquipment
{
	GENERATED_BODY()

public:

	USkyraGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	UFUNCTION(BlueprintCallable, Category="Skyra|Ability")
	USkyraRangedWeaponInstance* GetWeaponInstance() const;
...
protected:
	struct FRangedWeaponFiringInput
	{
		...
		USkyraRangedWeaponInstance* WeaponData = nullptr;
...
protected:
	...
	FTransform GetTargetingTransform(APawn* SourcePawn, ESkyraAbilityTargetingSource Source) const;

...
#include "SkyraReticleWidgetBase.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SkyraReticleWidgetBase)

USkyraReticleWidgetBase::USkyraReticleWidgetBase(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void USkyraReticleWidgetBase::InitializeFromWeapon(USkyraWeaponInstance* InWeapon)
{
	WeaponInstance = InWeapon;
	InventoryInstance = nullptr;
	if (WeaponInstance)
	{
		InventoryInstance = Cast<USkyraInventoryItemInstance>(WeaponInstance->GetInstigator());
	}
	OnWeaponInitialized();
}


float USkyraReticleWidgetBase::ComputeSpreadAngle() const
{
	if (const USkyraRangedWeaponInstance* RangedWeapon = Cast<const USkyraRangedWeaponInstance>(WeaponInstance))
	{
...
bool USkyraReticleWidgetBase::HasFirstShotAccuracy() const
{
	if (const USkyraRangedWeaponInstance* RangedWeapon = Cast<const USkyraRangedWeaponInstance>(WeaponInstance))
	{
...
float USkyraReticleWidgetBase::ComputeMaxScreenspaceSpreadRadius() const
{

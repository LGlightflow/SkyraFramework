...
#include "SkyraReticleWidgetBase.generated.h"
...
UCLASS(Abstract)
class USkyraReticleWidgetBase : public UCommonUserWidget
{
	GENERATED_BODY()

public:
	USkyraReticleWidgetBase(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	UFUNCTION(BlueprintImplementableEvent)
	void OnWeaponInitialized();

	UFUNCTION(BlueprintCallable)
	void InitializeFromWeapon(USkyraWeaponInstance* InWeapon);
...
protected:
	...
	TObjectPtr<USkyraWeaponInstance> WeaponInstance;

	UPROPERTY(BlueprintReadOnly)
	TObjectPtr<USkyraInventoryItemInstance> InventoryInstance;
};
